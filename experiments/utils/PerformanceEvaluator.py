...
class PerformanceEvaluator:
    ...
    def eval_detailed_latency(self, num_runs=100):
        ...
        for batch in tqdm(self.data_loader, desc="batches", unit="batch"):
    ...
    def throughput_eval(self, num_iterations=30):
        ...
        for batch in tqdm(self.data_loader, desc="throughput eval batches", unit="batch", total=self.n_batches):
    ...
    def latency_eval(self, max_samples=None):
        ...
        self.model.eval()
        lat_list = []
        for batch in tqdm(self.data_loader, desc="latency eval samples", total=(max_samples or self.n_batches)):
    ...
    def warm_up_cuda(self, n_batches=3):
        ...
        if self.cuda_allowed:
            for batch in tqdm(self.data_loader, desc="warming", total=n_batches):
                if isinstance(batch, tuple) or isinstance(batch, list):
                    inputs = batch[0]
